...
export function fetchPairToken0(pairAddress: Address): Address {
  let contract = Pair.bind(pairAddress)
  let token0Result = contract.try_token0()
  if (!token0Result.reverted) {
    return token0Result.value
  }
  return null
}

export function fetchPairToken1(pairAddress: Address): Address {
  let contract = Pair.bind(pairAddress)
  let token1Result = contract.try_token1()
  if (!token1Result.reverted) {
    return token1Result.value
  }
  return null
}
